...
1. 根据历史时序自动生成一段文本 prompt，包含最大/最小/平均/趋势/周期/滞后等时序特征的patch embedding。
2. 历史时序切成 patch后，和 LLM 词向量原型做 cross-attention。  
4. 得到 LLM 可以理解的 reprogrammed patch embedding，再加上一段自动生成的文本得到 prompt embedding。  
5. 自动生成的文本 prompt embedding 和原始 patch embedding 拼接。  
6. 输入冻结 LLM，最后取时序部分 hidden states 预测未来。 
