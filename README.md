...
它的动机是：如果文本真的和时序相关，那么文本表示的变化也可能带有时间规律。论文借用柏拉图表示假设证明文本与时序的相关性（周期相同）：不同模态的表示会收敛至共享空间。在此背景下，通过TT算法(cos余弦计算滞后相似度)计算发现时序配对文本的主导周期分量可能天然呈现与原始时序高度相似的周期性特征。因此，将时序配对文本视为时序的辅助变量，增强时序的周期特性。
...
![1.1.3.png](论文总结survey.assets/1.1.3.png)
...
![1.1.4.png](论文总结survey.assets/1.1.4.png)
