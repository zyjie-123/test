...
链接：https://arxiv.org/abs/2405.13522  
...
作用是让每个survey变量自己去找整段文本中与自己相关的字段。（模型学习“某个变量描述”应该关注哪些天气文本，每天的文本有7段，各被压缩为1个句向量，对应21个变量）
...
<img alt="intro" src="assets/image1.png">
...
3、一天有7段文本，每段文本几十个词被压缩为了1个文本向量（1,768），是否文本语义丢失严重（比TaTS丢失的少）。
...
<img alt="1.2.1" src="assets/1.2.1.png">

实验：

1. 周期验证：先定性观察文本和时序是否有相同主导周期，再定量计算二者周期表示之间的距离。距离越小，说明文本周期和时序周期越接近,打乱之后周期相似性差距变大。   

<img alt="1.2.2" src="assets/1.2.2.png">

3. 预测和补全实验：在不同预测长度下比较加入文本后的效果，并做缺失值补全任务。总体上，TT 值越小，说明文本和时序越对齐，模型提升越明显。
4. 文本消融实验：比较原文本、打乱文本、丢弃文本、打乱并丢弃文本。丢弃文本时用“无可用信息”替代。结果说明，有周期结构的原文本通常比打乱或丢弃文本更有帮助，但也存在少数不完全符合规律的样本（红色这几个没有这个规律）。   


<img alt="1.2.3" src="assets/1.2.3.png">

5. 超参数实验：不同文本投影维度的效果差别不大，说明方法对投影维度比较稳健。不同文本编码器下也能保持效果，编码器越大通常提升越明显，但文本编码器规模和预测提升之间的关系仍是开放问题：“探究文本编码器规模与 TaTS 有效性之间的关系仍是未来值得研究的开放方向。”。  
 
<img alt="1.2.4" src="assets/1.2.4.png">

6. 融合方式实验：论文比较了线性投影、门控残差和 cross-attention 三种融合方式，结果线性投影最好。这也说明更复杂的融合方式不一定更有效，后续还可以继续探索更细粒度的融合设计“将探索更细粒度的多模态融合设计作为未来有趣的研究方向”。
7. 开销与收益实验：作者还比较了额外计算开销和性能增益，说明加入文本会增加成本，但在文本和时序对齐较好时收益更明显。

主要启发：文本可以不只作为一句解释，而是可以转成随时间变化的辅助变量。

主要问题：  
1、把文本向量直接拼成变量，融合方式比较粗。模型不一定真的理解文本语义，只是学习文本向量和数值之间的相关性。  
2、把一整段文本几百个词压缩为了一个文本向量，把312维度的文本向量压缩为12维度的向量（1,12），两次压缩都很严重，文本语义是否丢失严重，是否丢失重要token权重。（是否可以使用cls或bos代表整段文本语义）
...
<img alt="image3-1" src="assets/image3-1.png">
<img alt="image3-2" src="assets/image3-2.png">
...
<img alt="image4" src="assets/image4.png">

动机：  
现有方案虽实现文本与时序融合，但文本融合在高维时序预测中的实际效果尚无系统性验证。  
高维时序预测主要指同时预测大量变量（channels>100）的时间序列，每个变量具有不同的动态模式。  

实验：  
1.消融实验：有/无文本模型  
2.3种不同文本提取方案：avg(取均值)、bos(transformer使用特殊符表示hBOS)、cls(transformer使用特殊符表示hCLS)；三种方案性能差距极小。  

主要启发：静态文本可以用来选择历史片段。例如一个新闻主题可能决定哪些反馈阶段更重要。

主要问题：  
1.文本是静态的，不描述未来变化，每次和时序对应的都是同一段文本。它更像“变量说明”或“对象描述”，不是事件预告。  
2.实验没有与其他模型作比较，没有benchmark。  
...
<img alt="image5" src="assets/image5.png">
...
链接：https://ojs.aaai.org/index.php/AAAI/article/view/30383  
...
seq_len=15,patch_num=3,patch_size=8,dmodel=768,C=变量数。  
流程是：

1. 每个时间点历史文本经过 BERT 编码生成1个768维文本向量（hCLS）：`[batch,text_len]-> [batch, 1, 768]*15*C->[batch*C, 15, 768]->[batch*C, 3, 768]`。
2. 所有文本向量会对其时序patch切分方法也被压缩成少量 soft prompt。
3. 历史时序被切成 patch，并映射到和 GPT-2 hidden size 对齐的向量维度:`[batch,15,C]->[batch*C,3,8]->[batch*C, 3, 768]`。
4. soft prompt和时序patch token拼在一起送入GPT-2:`[batch*C,3,768]concat[batch*C,3,768]=[batch*C,6,768]->[batch*C,3,768]只取GPT2的时序部分输出`。
5. GPT-2 输出后，只取时序部分 hidden states，再用线性层预测未来时序:`[batch*C,3,768]->[batch, pre_len, C]`。  

 
<img alt="image6" src="assets/image6.png">

这里的 soft prompt 可以理解成“不是人写的提示词，而是模型学出来的一组文本提示向量”。它不直接可读，但能影响 GPT-2 的注意力。

动机：  
1.尽管额外辅助信息能够提升时序预测效果，但信息采集难度较大：提出一套借助现代大语言模型、从多数据源采集配套文本信息的通用流程。  
2.多模态信息的融合建模本身是一项极具挑战性的难题：基于提示词的大语言模型框架 GPT4MTS，可同时利用数值与时序文本两类信息完成预测。  
3.要么将时序直接转换为文本序列输入，要么仅把时序嵌入与大语言模型文本嵌入做对齐：同时输入时序、文本两种模态信息。  
4.创新点：在于利用文本提示引导模型处理时序数值信息。仅保留时序隐状态作为输出特征是本文模型的关键优化点。  
5.新闻传播学：预测事件受关注度。本文提供通用多模态数据集搭建流程，并基于新闻场景落地，数据集与模型均可作为后续多模态时序研究基础。  

实验：  
1.主实验：与其他9类模型对比10个数据集的结果。  
2.消融：无文本、同时使用文本 + 时序全部隐状态进入线性层输出结果和仅保留时序隐状态3个对照实验。  
结论：模型性能提升来源于数值时序、文本信息、提示融合方案三者协同作用。消融实验同时说明文本仅作为引导特征，不应作为输出主干。过度依赖文本会干扰数值预测精度。  
...
3、文本只是使用历史文本，改成未来文本？  
4、数据集时序规律简单，Transformer过拟合，普通线性模型效果普遍优于复杂Transformer。
## 3.2、Time-LLM
...
<img alt="image7" src="assets/image7.png">
...
<img alt="xTP" src="assets/xTP-LLM.JPG">
...
下一步工作突破点：  
1.多变量数据集较少，仅有GPT4MTS、TGForecaster、TextFusionHTS，但文本都没显示指明变量关系，可以从多变量关系建模和预测方面进行突破。
