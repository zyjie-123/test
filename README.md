...
3. 文本向量被当作额外变量，拼到原始时序变量后面，变成` time*([batch,1,1]+[batch,1,12])=[batch,time,13]`。
4. 扩展后的输入交给基础时序模型预测未来`[batch,time,13]->[batch*time,13,dmodel]`。
5. 预测结果再和 `prior_history_avg` 加权融合。
...
它的动机是：如果文本真的和时序相关，那么文本表示的变化也可能带有时间规律。论文借用 Plato Representation Hypothesis 的想法，认为不同模态描述同一对象时，可能会收敛到相似的表示空间。因此，配对文本不只是背景说明，也可以看成一种随时间变化的辅助信号。

论文提出 TT 方法，用余弦相似度比较文本变化和时序变化的主导周期是否相近。比如在月度数据中，原始时序有 12 个月周期，配对文本也可能出现周期为 12、频率约为 0.083 的特征。这说明文本和时序可能受到共同外部因素驱动，也可能是时序变化影响了文本内容，或者文本里包含了和时序周期对齐的额外变量。

实验验证主要分几类：

1. 周期验证：先定性观察文本和时序是否有相同主导周期，再定量计算二者周期表示之间的距离。距离越小，说明文本周期和时序周期越接近。
2. 预测和补全实验：在不同预测长度下比较加入文本后的效果，并做缺失值补全任务。总体上，TT 值越小，说明文本和时序越对齐，模型提升越明显。
3. 文本消融实验：比较原文本、打乱文本、丢弃文本、打乱并丢弃文本。丢弃文本时用“无可用信息”替代。结果说明，有周期结构的原文本通常比打乱或丢弃文本更有帮助，但也存在少数不完全符合规律的样本。
4. 超参数实验：不同文本投影维度的效果差别不大，说明方法对投影维度比较稳健。不同文本编码器下也能保持效果，编码器越大通常提升越明显，但文本编码器规模和预测提升之间的关系仍是开放问题。
5. 融合方式实验：论文比较了线性投影、门控残差和 cross-attention 三种融合方式，结果线性投影最好。这也说明更复杂的融合方式不一定更有效，后续还可以继续探索更细粒度的融合设计。
